...
export function useActiveEffects() {
  ...
  const sortedEffects = useMemo(() => {
    ...
    if (strand.strandWebEnabled) activeEffects.push({ id: 'strand_web', label: 'Strand Web', color: '#707070', primaryValue: Math.round(strand.strandWebParams.threshold * 100), primaryLabel: 'thresh' })
    if (strand.bridgeLinkEnabled) activeEffects.push({ id: 'strand_bridge', label: 'Bridge', color: '#707070', primaryValue: strand.bridgeLinkParams.gridSize, primaryLabel: 'grid' })
    if (strand.chiralPathEnabled) activeEffects.push({ id: 'strand_path', label: 'Chiral Path', color: '#707070', primaryValue: strand.chiralPathParams.particleCount, primaryLabel: 'cnt' })
    if (strand.umbilicalEnabled) activeEffects.push({ id: 'strand_umbilical', label: 'Umbilical', color: '#707070', primaryValue: strand.umbilicalParams.tendrilCount, primaryLabel: 'cnt' })
